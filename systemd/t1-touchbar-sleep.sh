#!/bin/sh
# /usr/lib/systemd/system-sleep/t1-touchbar
#
...
# dfr-probe holds the display session open by continuously streaming frames,
...
case "$1" in
    pre)
        logger -t t1-touchbar-sleep "stopping the display session before $2"
        timeout 60 systemctl stop t1-touchbar-bar.service t1-touchbar-display.service
        ...
        timeout 15 rmmod dfr_probe 2>/dev/null
        logger -t t1-touchbar-sleep "display session down"
        ;;
    post)
        logger -t t1-touchbar-sleep "restoring the display session after $2"
        timeout 120 systemctl start t1-touchbar-display.service
        timeout 60  systemctl start t1-touchbar-bar.service
        logger -t t1-touchbar-sleep "display session restored"
        ;;
esac
exit 0
